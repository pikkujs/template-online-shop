...
-- The shop's own customer profile. Better Auth owns identity and credentials
-- in its `user` table; user_id mirrors that row's id, so the shop can add its
-- own columns without touching a table it does not control.
